...
//  Puyo Puyo
/*
 * bfs를 이용하여 같은 색을 찾고, 이미 체크한 칸인지 2차원 boolean 배열을 만들어서 확인
...
public class Baekjoon_11559 {
	...
	public static void play() {
		while(true) {
			...
			if(!isPopped) break; // 터지지 않으면 연쇄가 끊어지므로 바로 종료
			// 터지면
			combo++; // 연쇄횟수 증가
	...
	public static void columnDown(int col) { // 한 열 내리기
		...
		for(int i=11; i>=0; i--) { // 바닥에서부터 위로 큐에 색깔만 집어넣기
			char ch = arr[i][col];
	...
	public static void popColor(int r, int c) { // 좌표 기준으로 같은 색 4개 이상이면 터트리는 메소드
		if(arr[r][c] == '.') return;
		char color = arr[r][c];
		searchColor(r,c,color); // 큐에 같은 색들의 좌표 저장
	...
	public static void searchColor(int r, int c, char color) { // bfs이용하여 해당좌표와 붙어있는 같은 색들의 좌표를 큐에 저장
		// 재귀함수 이용
		if(arr[r][c] != color) { // 현재 칸이 탐색중인 색깔과 다르면 종료
			return;
